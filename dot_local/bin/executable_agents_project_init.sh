#!/usr/bin/env bash
# agents_project_init.sh - Initialize AGENTS files for a project
...
printf "${BOLD}🔧 Initializing AGENTS files for project${RESET}\n\n"

# Check we're in a reasonable directory
if [ ! -d .git ] && [ ! -d .jj ]; then
    printf "${RED}❌ Error: Not in a git or jj repository root${RESET}\n"
...
**Last Updated:** $TODAY
**Refresh By:** $REFRESH_BY  (run agents_project_init.sh daily, update this date when refreshing)
...
if [ -L AGENTS.md ]; then
    ...
    printf "   ${GREEN}✅ Created AGENTS.md → AGENTS.local.md${RESET}\n"
...
# AGENTS files (added by agents_project_init.sh)
...
if [ "$IS_REFRESH" = true ]; then
    ...
    if [ -n "$GLOBAL_UPDATED" ]; then
        printf "${YELLOW}⚠️  ~/AGENTS.global.md was updated $GLOBAL_UPDATED${RESET}\n"
        printf "   Consider reviewing for new conventions or template changes.\n\n"
    fi

    if [ ! -f THIS_BRANCH.md ]; then
        printf "${BOLD}🤖 AGENTS: Check if you should create THIS_BRANCH.md for current work${RESET}\n"
        ...
        printf "   ${BLUE}If yes:${RESET} Run agents_task_init.sh to bootstrap it\n\n"
    fi
